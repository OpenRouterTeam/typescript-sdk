...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.CreateAudioTranscriptionsMultipartRequest$outboundSchema.parse(
        value,
      ),
  ...
  if (isBlobLike(payload.STTMultipartRequest.file)) {
    const file = payload.STTMultipartRequest.file;
    const blob = await normalizeBlob(file);
    const name = "name" in file ? (file.name as string) : undefined;
    appendForm(body, "file", blob, name);
  } else if (isReadableStream(payload.STTMultipartRequest.file.content)) {
    const buffer = await readableStreamToArrayBuffer(
      payload.STTMultipartRequest.file.content,
    );
    const contentType =
      getContentTypeFromFileName(payload.STTMultipartRequest.file.fileName)
      || "application/octet-stream";
    appendForm(
      body,
      "file",
      bytesToBlob(buffer, contentType),
      payload.STTMultipartRequest.file.fileName,
    );
  } else {
    const contentType =
      getContentTypeFromFileName(payload.STTMultipartRequest.file.fileName)
    ...
    appendForm(
      body,
      "file",
      bytesToBlob(payload.STTMultipartRequest.file.content, contentType),
      payload.STTMultipartRequest.file.fileName,
    );
  }
  appendForm(body, "model", payload.STTMultipartRequest.model);
  if (payload.STTMultipartRequest.language !== undefined) {
    appendForm(body, "language", payload.STTMultipartRequest.language);
  }
  if (payload.STTMultipartRequest.temperature !== undefined) {
    appendForm(body, "temperature", payload.STTMultipartRequest.temperature);
  }
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, models.STTResponse$inboundSchema),

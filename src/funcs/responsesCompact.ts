...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.CreateResponsesCompactRequest$outboundSchema.parse(value),
  ...
  const body = encodeJSON("body", payload.CompactRequest, { explode: true });

  const path = pathToFunc("/responses/compact")();
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, models.CompactResponse$inboundSchema),

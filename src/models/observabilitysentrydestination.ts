/*
 * Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT.
 * @generated-id: 05cd77bde6a0
 */

import * as z from "zod/v4";
import { remap as remap$ } from "../lib/primitives.js";
import { safeParse } from "../lib/schemas.js";
import * as openEnums from "../types/enums.js";
import { OpenEnum } from "../types/enums.js";
import { Result as SafeParseResult } from "../types/fp.js";
import { SDKValidationError } from "./errors/sdkvalidationerror.js";
import {
  ObservabilityFilterRulesConfig,
  ObservabilityFilterRulesConfig$inboundSchema,
} from "./observabilityfilterrulesconfig.js";

export type ObservabilitySentryDestinationConfig = {
  dsn: string;
  /**
   * Custom HTTP headers to include in requests to this destination.
   */
  headers?: { [k: string]: string } | undefined;
  otlpEndpoint: string;
};

export const ObservabilitySentryDestinationRegion = {
  Global: "global",
  Europe: "europe",
  Us: "us",
} as const;
export type ObservabilitySentryDestinationRegion = OpenEnum<
  typeof ObservabilitySentryDestinationRegion
>;

export type ObservabilitySentryDestination = {
  /**
   * Optional allowlist of OpenRouter API key hashes (`api_keys.hash`) whose traffic is forwarded to this destination. `null` means all keys.
   */
  apiKeyHashes: Array<string> | null;
  /**
   * When true, include cost and billing generation metadata.
   */
  broadcastGenerationCost: boolean;
  /**
   * When true, include identity generation metadata.
   */
  broadcastGenerationIdentity: boolean;
  /**
   * When true, include request-context generation metadata.
   */
  broadcastGenerationRequestContext: boolean;
  config: ObservabilitySentryDestinationConfig;
  /**
   * ISO timestamp of when the destination was created.
   */
  createdAt: string;
  /**
   * Whether this destination is currently enabled.
   */
  enabled: boolean;
  /**
   * Optional structured filter rules controlling which events are forwarded.
   */
  filterRules: ObservabilityFilterRulesConfig | null;
  /**
   * Stable public identifier for this destination.
   */
  id: string;
  /**
   * Human-readable name for the destination.
   */
  name: string | null;
  /**
   * When true, request/response bodies are not forwarded to this destination — only metadata.
   */
  privacyMode: boolean;
  /**
   * Data regions this destination applies to. Requests served in a region only fan out to destinations that include that region.
   */
  regions: Array<ObservabilitySentryDestinationRegion>;
  /**
   * Sampling rate for events sent to this destination, between 0.0001 and 1 (1 = 100%).
   */
  samplingRate: number;
  type: "sentry";
  /**
   * ISO timestamp of when the destination was last updated.
   */
  updatedAt: string;
  /**
   * ID of the workspace this destination belongs to.
   */
  workspaceId: string;
};

/** @internal */
export const ObservabilitySentryDestinationConfig$inboundSchema: z.ZodType<
  ObservabilitySentryDestinationConfig,
  unknown
> = z.object({
  dsn: z.string(),
  headers: z.record(z.string(), z.string()).optional(),
  otlpEndpoint: z.string(),
});

export function observabilitySentryDestinationConfigFromJSON(
  jsonString: string,
): SafeParseResult<ObservabilitySentryDestinationConfig, SDKValidationError> {
  return safeParse(
    jsonString,
    (x) =>
      ObservabilitySentryDestinationConfig$inboundSchema.parse(JSON.parse(x)),
    `Failed to parse 'ObservabilitySentryDestinationConfig' from JSON`,
  );
}

/** @internal */
export const ObservabilitySentryDestinationRegion$inboundSchema: z.ZodType<
  ObservabilitySentryDestinationRegion,
  unknown
> = openEnums.inboundSchema(ObservabilitySentryDestinationRegion);

/** @internal */
export const ObservabilitySentryDestination$inboundSchema: z.ZodType<
  ObservabilitySentryDestination,
  unknown
> = z.object({
  api_key_hashes: z.nullable(z.array(z.string())),
  broadcast_generation_cost: z.boolean(),
  broadcast_generation_identity: z.boolean(),
  broadcast_generation_request_context: z.boolean(),
  config: z.lazy(() => ObservabilitySentryDestinationConfig$inboundSchema),
  created_at: z.string(),
  enabled: z.boolean(),
  filter_rules: z.nullable(ObservabilityFilterRulesConfig$inboundSchema),
  id: z.string(),
  name: z.nullable(z.string()),
  privacy_mode: z.boolean(),
  regions: z.array(ObservabilitySentryDestinationRegion$inboundSchema),
  sampling_rate: z.number(),
  type: z.literal("sentry"),
  updated_at: z.string(),
  workspace_id: z.string(),
}).transform((v) => {
  return remap$(v, {
    "api_key_hashes": "apiKeyHashes",
    "broadcast_generation_cost": "broadcastGenerationCost",
    "broadcast_generation_identity": "broadcastGenerationIdentity",
    "broadcast_generation_request_context": "broadcastGenerationRequestContext",
    "created_at": "createdAt",
    "filter_rules": "filterRules",
    "privacy_mode": "privacyMode",
    "sampling_rate": "samplingRate",
    "updated_at": "updatedAt",
    "workspace_id": "workspaceId",
  });
});

export function observabilitySentryDestinationFromJSON(
  jsonString: string,
): SafeParseResult<ObservabilitySentryDestination, SDKValidationError> {
  return safeParse(
    jsonString,
    (x) => ObservabilitySentryDestination$inboundSchema.parse(JSON.parse(x)),
    `Failed to parse 'ObservabilitySentryDestination' from JSON`,
  );
}

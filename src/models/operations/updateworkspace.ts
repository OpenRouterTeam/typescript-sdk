/*
 * Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT.
 * @generated-id: b10c83b1ce4f
 */

import * as z from "zod/v4";
import { remap as remap$ } from "../../lib/primitives.js";
import * as models from "../index.js";

export type UpdateWorkspaceGlobals = {
  /**
   * The app identifier should be your app's URL and is used as the primary identifier for rankings.
   *
   * @remarks
   * This is used to track API usage per application.
   */
  httpReferer?: string | undefined;
  /**
   * The app display name allows you to customize how your app appears in OpenRouter's dashboard.
   *
   * @remarks
   */
  appTitle?: string | undefined;
  /**
   * Comma-separated list of app categories (e.g. "cli-agent,cloud-agent"). Used for marketplace rankings.
   *
   * @remarks
   */
  appCategories?: string | undefined;
};

export type UpdateWorkspaceSecurity = {
  managementKey: string;
};

export type UpdateWorkspaceRequest = {
  /**
   * The app identifier should be your app's URL and is used as the primary identifier for rankings.
   *
   * @remarks
   * This is used to track API usage per application.
   */
  httpReferer?: string | undefined;
  /**
   * The app display name allows you to customize how your app appears in OpenRouter's dashboard.
   *
   * @remarks
   */
  appTitle?: string | undefined;
  /**
   * Comma-separated list of app categories (e.g. "cli-agent,cloud-agent"). Used for marketplace rankings.
   *
   * @remarks
   */
  appCategories?: string | undefined;
  /**
   * The workspace ID (UUID) or slug
   */
  id: string;
  updateWorkspaceRequest: models.UpdateWorkspaceRequest;
};

/** @internal */
export type UpdateWorkspaceSecurity$Outbound = {
  managementKey: string;
};

/** @internal */
export const UpdateWorkspaceSecurity$outboundSchema: z.ZodType<
  UpdateWorkspaceSecurity$Outbound,
  UpdateWorkspaceSecurity
> = z.object({
  managementKey: z.string(),
});

export function updateWorkspaceSecurityToJSON(
  updateWorkspaceSecurity: UpdateWorkspaceSecurity,
): string {
  return JSON.stringify(
    UpdateWorkspaceSecurity$outboundSchema.parse(updateWorkspaceSecurity),
  );
}

/** @internal */
export type UpdateWorkspaceRequest$Outbound = {
  "HTTP-Referer"?: string | undefined;
  appTitle?: string | undefined;
  appCategories?: string | undefined;
  id: string;
  UpdateWorkspaceRequest: models.UpdateWorkspaceRequest$Outbound;
};

/** @internal */
export const UpdateWorkspaceRequest$outboundSchema: z.ZodType<
  UpdateWorkspaceRequest$Outbound,
  UpdateWorkspaceRequest
> = z.object({
  httpReferer: z.string().optional(),
  appTitle: z.string().optional(),
  appCategories: z.string().optional(),
  id: z.string(),
  updateWorkspaceRequest: models.UpdateWorkspaceRequest$outboundSchema,
}).transform((v) => {
  return remap$(v, {
    httpReferer: "HTTP-Referer",
    updateWorkspaceRequest: "UpdateWorkspaceRequest",
  });
});

export function updateWorkspaceRequestToJSON(
  updateWorkspaceRequest: UpdateWorkspaceRequest,
): string {
  return JSON.stringify(
    UpdateWorkspaceRequest$outboundSchema.parse(updateWorkspaceRequest),
  );
}

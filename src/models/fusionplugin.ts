...
export type FusionPlugin = {
  ...
  /**
   * Branded OpenRouter fusion preset slug (e.g. "or-coding-high") following `or-<task>-<tier>`. Expands server-side into the preset's analysis_models panel and judge model, so callers never name individual models. Explicitly provided `analysis_models` / `model` take precedence; an unknown slug falls back to the default panel.
   */

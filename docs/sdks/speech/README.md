...
async function run() {
  const result = await openRouter.speech.createSpeech({
    speechRequest: {
      input: "Hello world",
...
import { speechCreateSpeech } from "@openrouter/sdk/funcs/speechCreateSpeech.js";
...
async function run() {
  const res = await speechCreateSpeech(openRouter, {
    speechRequest: {
      input: "Hello world",
      model: "elevenlabs/eleven-turbo-v2",
      speed: 1,
  ...
  if (res.ok) {
    ...
    console.log("speechCreateSpeech failed:", res.error);
  }
}

---
title: "MessagesMessageParamContentUnion4"
---

## Supported Types

### `models.AnthropicTextBlockParam`

```typescript
const value: models.AnthropicTextBlockParam = {
  text: "Hello, world!",
  type: "text",
};
```

### `models.AnthropicImageBlockParam`

```typescript
const value: models.AnthropicImageBlockParam = {
  source: {
    data: "/9j/4AAQ...",
    mediaType: "image/jpeg",
    type: "base64",
  },
  type: "image",
};
```

### `models.AnthropicDocumentBlockParam`

```typescript
const value: models.AnthropicDocumentBlockParam = {
  source: {
    data: "Hello, world!",
    mediaType: "text/plain",
    type: "text",
  },
  type: "document",
};
```

### `models.ContentToolUse`

```typescript
const value: models.ContentToolUse = {
  id: "<id>",
  name: "<value>",
  type: "tool_use",
};
```

### `models.ContentToolResult`

```typescript
const value: models.ContentToolResult = {
  toolUseId: "<id>",
  type: "tool_result",
};
```

### `models.ContentThinking`

```typescript
const value: models.ContentThinking = {
  signature: "<value>",
  thinking: "<value>",
  type: "thinking",
};
```

### `models.ContentRedactedThinking`

```typescript
const value: models.ContentRedactedThinking = {
  data: "<value>",
  type: "redacted_thinking",
};
```

### `models.ContentServerToolUse`

```typescript
const value: models.ContentServerToolUse = {
  id: "<id>",
  name: "<value>",
  type: "server_tool_use",
};
```

### `models.ContentWebSearchToolResult`

```typescript
const value: models.ContentWebSearchToolResult = {
  content: [
    {
      encryptedContent: "enc_content_0",
      title: "Example Page",
      type: "web_search_result",
      url: "https://example.com",
    },
  ],
  toolUseId: "<id>",
  type: "web_search_tool_result",
};
```

### `models.AnthropicSearchResultBlockParam`

```typescript
const value: models.AnthropicSearchResultBlockParam = {
  content: [
    {
      text: "Result content",
      type: "text",
    },
  ],
  source: "example_source",
  title: "Example Result",
  type: "search_result",
};
```

### `models.ContentCompaction`

```typescript
const value: models.ContentCompaction = {
  content: null,
  type: "compaction",
};
```

### `models.MessagesAdvisorToolResultBlock`

```typescript
const value: models.MessagesAdvisorToolResultBlock = {
  content: {
    "text": "Advisor response text",
    "type": "advisor_result",
  },
  toolUseId: "srvtoolu_01abc",
  type: "advisor_tool_result",
};
```

### `models.MessagesToolAdditionBlock`

```typescript
const value: models.MessagesToolAdditionBlock = {
  tool: {
    name: "get_forecast",
    type: "tool_reference",
  },
  type: "tool_addition",
};
```

### `models.MessagesToolRemovalBlock`

```typescript
const value: models.MessagesToolRemovalBlock = {
  tool: {
    name: "get_weather",
    type: "tool_reference",
  },
  type: "tool_removal",
};
```

### `models.MessagesShellToolResultBlock`

```typescript
const value: models.MessagesShellToolResultBlock = {
  content: {
    "output": [
      {
        "outcome": {
          "exit_code": 0,
          "type": "exit",
        },
        "stderr": "",
        "stdout": "README.md\n",
      },
    ],
  },
  toolUseId: "srvtoolu_01abc",
  type: "openrouter_shell_tool_result",
};
```

### `models.MessagesBashToolResultBlock`

```typescript
const value: models.MessagesBashToolResultBlock = {
  content: {
    "command": "ls",
    "exitCode": 0,
    "stderr": "",
    "stdout": "README.md\n",
  },
  toolUseId: "srvtoolu_01abc",
  type: "openrouter_bash_tool_result",
};
```

### `models.MessagesMcpToolUseBlockParam`

```typescript
const value: models.MessagesMcpToolUseBlockParam = {
  id: "srvtoolu_01abc",
  name: "web_search",
  serverName: "openrouter",
  type: "mcp_tool_use",
};
```

### `models.MessagesMcpToolResultBlockParam`

```typescript
const value: models.MessagesMcpToolResultBlockParam = {
  toolUseId: "srvtoolu_01abc",
  type: "mcp_tool_result",
};
```


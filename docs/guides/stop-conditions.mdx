...
subtitle: Control execution with intelligent stopping conditions
headline: 'Stop Conditions | OpenRouter SDK'
canonical-url: https://openrouter.ai/docs/guides/stop-conditions
og:site_name: OpenRouter Documentation
og:title: 'Stop Conditions - OpenRouter SDK'
og:description: 'Control multi-turn execution with stopWhen. Use built-in helpers or custom conditions to stop by step count, tool calls, cost, or tokens.'
og:image: https://openrouter.ai/dynamic-og?title=Stop%20Conditions&description=Intelligent%20Execution%20Control
og:image:width: 1200
og:image:height: 630
...
The `stopWhen` parameter provides intelligent control over multi-turn execution. Instead of simple round limits, you can stop based on tool calls, token usage, cost, or any custom condition.
...
const result = openrouter.callModel({
  ...
  input: 'Process data',
  tools: [processTool],
  stopWhen: ({ steps, totalTokens, totalCost }) => {
    // Stop after 20 steps
...
|----------|------|-------------|
| `steps` | `StepResult[]` | All completed steps |
| `totalTokens` | `number` | Total tokens used |
| `totalCost` | `number` | Total cost incurred |
| `lastResponse` | `Response` | Most recent response |
...
- **[Dynamic Parameters](/docs/guides/dynamic-parameters)** - Adaptive behavior
- **[nextTurnParams](/docs/guides/next-turn-params)** - Tool-driven modifications

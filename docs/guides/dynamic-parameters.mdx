...
canonical-url: https://openrouter.ai/docs/guides/dynamic-parameters
og:site_name: OpenRouter Documentation
og:title: 'Dynamic Parameters - OpenRouter SDK'
og:description: 'Use async functions to dynamically compute callModel parameters. Adapt model selection, temperature, and instructions based on conversation state.'
og:image: https://openrouter.ai/dynamic-og?title=Dynamic%20Parameters&description=Adaptive%20Model%20Behavior
og:image:width: 1200
og:image:height: 630
...
const result = openrouter.callModel({
  ...
  model: (ctx) => {
    return ctx.numberOfTurns > 3 ? 'openai/gpt-4' : 'openai/gpt-4o-mini';
  },
  input: 'Hello!',
  tools: [myTool],
});
...
| `model` | `string \| undefined` | Model (if static) |
| `models` | `string[] \| undefined` | Models array (if static) |
...
const result = openrouter.callModel({
  ...
  temperature: (ctx) => {
    ...
    const lastMessage = JSON.stringify(ctx.messageHistory).toLowerCase();
...
const smartAssistant = openrouter.callModel({
  ...
  model: (ctx) => {
    const hasToolUse = JSON.stringify(ctx.messageHistory).includes('function_call');
    return hasToolUse ? 'anthropic/claude-sonnet-4' : 'openai/gpt-4o-mini';
  },
...
model: (ctx) => {
  ...
  return preferredModel ?? 'openai/gpt-4o-mini'; // Default fallback
},
```

## See Also

- **[nextTurnParams](/docs/guides/next-turn-params)** - Tool-driven parameter modification
- **[Stop Conditions](/docs/guides/stop-conditions)** - Dynamic execution control

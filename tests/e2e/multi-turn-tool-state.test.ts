...
import type { InputsUnion, InputsUnion1 } from '../../src/models/inputsunion.js';
...
describe('Multi-Turn Tool State E2E Tests', () => {
  ...
  const createRequestInspector = (): BeforeRequestHook => ({
    beforeRequest: async (_hookCtx: BeforeRequestContext, request: Request) => {
      ...
      try {
        ...
        // Capture input whether it's an array or string
        const input = body.input;
        capturedRequests.push({
          input: Array.isArray(input) ? input : (typeof input === 'string' ? input : []),
          timestamp: Date.now(),
        });
      } catch {
        // Non-JSON body, skip
  ...
  /**
   * Check if an input item matches the expected criteria.
   */
  function matchesExpected(item: InputItem, expected: ExpectedInputItem): boolean {
    if (expected.role && item.role !== expected.role) {
      return false;
    }
    if (expected.content && item.content !== expected.content) {
      return false;
    }
    if (expected.type && item.type !== expected.type) {
      return false;
    }
    return true;
  }
  ...
  /**
   * Verify that expected input items appear in captured API requests.
   * Checks both role-based messages and type-based items.
   */
  function verifyInputItemsPreserved(
    requests: CapturedRequest[],
    expectedItems: ExpectedInputItem[],
    options: {
      fromRequestIndex?: number;
      checkAllRequests?: boolean;
    } = {},
  ): void {
    const { fromRequestIndex = 0, checkAllRequests = true } = options;
    const requestsToCheck = requests.slice(fromRequestIndex);

    for (const expected of expectedItems) {
      if (checkAllRequests) {
        // Expected item must be in EVERY request
        for (const request of requestsToCheck) {
          const items = request.input as InputItem[];
          const found = items.some((item) => matchesExpected(item, expected));
          expect(found, `Expected item ${JSON.stringify(expected)} missing from request`).toBe(true);
        }
      } else {
        // Expected item must be in at least ONE request
        const foundInAny = requestsToCheck.some((request) => {
          const items = request.input as InputItem[];
          return items.some((item) => matchesExpected(item, expected));
        });
        expect(foundInAny, `Expected item ${JSON.stringify(expected)} not found in any request`).toBe(true);
      }
    }
  }
  ...
  describe('Multi-Turn Conversation with Async Input Function', () => {
    it('should maintain cumulative item map with input as function', async () => {
      ...
      const inputFn = async (ctx: TurnContext): Promise<InputsUnion> => {
        ...
        const prevMessages: InputsUnion1[] = typeof prevInput === 'string' ? [] : (prevInput ?? []);
  ...
  //#region User Input Preservation Tests

  describe('User Input Preservation', () => {
    it('should preserve original user input in ALL API requests during internal multi-turn tool execution', async () => {
      const tools = [createCreateTaskTool(), createListTasksTool()];
      const originalUserMessage = 'First create task "Alpha", then list all tasks using list_tasks tool';

      const result = client.callModel({
        model: 'anthropic/claude-haiku-4.5',
        input: [{ role: 'user', content: originalUserMessage }],
        tools,
        stopWhen: stepCountIs(10),
      });
      ...
      // Should have multiple API requests (at least 2: initial + after tool execution)
      expect(capturedRequests.length).toBeGreaterThan(1);

      // Verify original user message is in EVERY request (not just the first)
      verifyInputItemsPreserved(
        capturedRequests,
        [{ role: 'user', content: originalUserMessage }],
        { checkAllRequests: true },
      );
    }, 120000);

    it('should normalize and preserve string input in follow-up requests', async () => {
      const tools = [createCreateTaskTool(), createListTasksTool()];
      const stringInput = 'Create task "StringTest" then list all tasks';

      const result = client.callModel({
        model: 'anthropic/claude-haiku-4.5',
        input: stringInput, // String input, not array
        tools,
        stopWhen: stepCountIs(10),
      });
      ...
      // Verify the string input is preserved in ALL requests
      for (let i = 0; i < capturedRequests.length; i++) {
        ...
        if (typeof input === 'string') {
          // First request may keep input as string
          hasOriginalContent = input.includes('StringTest');
        } else if (Array.isArray(input)) {
          // Follow-up requests normalize to array with user message
          const items = input as InputItem[];
          const userMessages = items.filter((item) => item.role === 'user');

          hasOriginalContent = userMessages.some((m) => {
            // Content could be a string or an array with input_text
            if (typeof m.content === 'string') {
              return m.content.includes('StringTest');
            }
            if (Array.isArray(m.content)) {
              return m.content.some(
                (c: { type?: string; text?: string }) =>
                  c.type === 'input_text' && c.text?.includes('StringTest'),
              );
            }
            return false;
          });
        ...
        expect(hasOriginalContent, `String input not preserved in request ${i}`).toBe(true);
      }
    }, 120000);

    it('should preserve all user messages across multiple callModel calls', async () => {
      const tools = [createCreateTaskTool(), createListTasksTool()];
      const conversationInput: Array<{ role: string; content: string }> = [];

      // === Call 1: First user message ===
      conversationInput.push({ role: 'user', content: 'Create a task named "First Task"' });
      ...
      const text1 = await result1.getText();
      conversationInput.push({ role: 'assistant', content: text1 });

      const call1RequestCount = capturedRequests.length;

      // === Call 2: Second user message (after model finished) ===
      conversationInput.push({ role: 'user', content: 'Now create another task named "Second Task"' });
      ...
      // Verify Call 2 requests contain BOTH user messages
      verifyInputItemsPreserved(
        capturedRequests,
        [
          { role: 'user', content: 'Create a task named "First Task"' },
          { role: 'user', content: 'Now create another task named "Second Task"' },
        ],
        { fromRequestIndex: call1RequestCount, checkAllRequests: true },
      );

      const call2Requests = capturedRequests.slice(call1RequestCount);
      expect(call2Requests.length).toBeGreaterThan(0);
    }, 120000);

    it('should preserve all inputs across multiple callModel calls, each with internal multi-turn execution', async () => {
      const tools = [createCreateTaskTool(), createListTasksTool(), createDeleteTaskTool()];
      const conversationInput: Array<{ role: string; content: string }> = [];

      // === Call 1: Creates tasks (triggers internal multi-turn) ===
      conversationInput.push({
        role: 'user',
        content: 'Create two tasks: "Task Alpha" and "Task Beta". Use create_task for each.',
      });
      ...
      const call1RequestCount = capturedRequests.length;
      expect(call1RequestCount).toBeGreaterThan(1); // Should have multiple internal turns

      // Verify Call 1: user message in all requests
      verifyInputItemsPreserved(
        capturedRequests.slice(0, call1RequestCount),
        [{ role: 'user', content: 'Create two tasks: "Task Alpha" and "Task Beta". Use create_task for each.' }],
        { checkAllRequests: true },
      );

      // === Call 2: List and delete (new user message + internal multi-turn) ===
      conversationInput.push({
        role: 'user',
        content: 'List all tasks with list_tasks, then delete "Task Alpha" with delete_task.',
      });
      ...
      const call2Requests = capturedRequests.slice(call1RequestCount);
      expect(call2Requests.length).toBeGreaterThan(1); // Should have multiple internal turns

      // Verify Call 2: BOTH user messages present in ALL Call 2 requests
      for (const request of call2Requests) {
        const items = request.input as InputItem[];
        const userMessages = items.filter((i) => i.role === 'user');

        // Must have both the original and the new user message
        expect(
          userMessages.some((m) => m.content?.includes('Create two tasks')),
          'Original user message from Call 1 missing',
        ).toBe(true);
        expect(
          userMessages.some((m) => m.content?.includes('List all tasks')),
          'New user message from Call 2 missing',
        ).toBe(true);
      }
    }, 180000);
  });
